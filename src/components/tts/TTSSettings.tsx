...
/**
 * TTS Direct Settings (no submenu wrapper)
 * For use in main menu when we want settings to appear directly
 */
export function TTSDirectSettings() {
  // TTS is only available in local development mode
  if (!import.meta.env.DEV) {
    return null;
  }
  ...
  // Check availability and load voices when component mounts
  useEffect(() => {
    if (settings.enabled) {
      setChecking(true);
      checkTTSAvailable().then(isAvailable => {
